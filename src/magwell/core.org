...
* Model
- This function follows a clear add → then subtract pattern:

** 1.Base solid
•Starts from the main shell (shell-with-slant-cut).
•Subtracts all internal cavities and interface cutouts that must exist inside the shell.

*** Primary shell shaping
- [[file:mag-inner.org][mag-inner-cutter]] – clears the internal volume for magazine insertion.
